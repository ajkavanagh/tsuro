...
filteredTiles (x:xs) = x : filteredTiles (filter f xs)
  where
    f n = n `notElem` rotatedTilesSet x
...
rotatedTilesSet xs = iterate rotateTile xs |> take 4 |> sort |> nub
...
generateAllTiles ps = expandSegments [] ps |> chunksOf 4 |> map concat
...
expandSegments xs ps = concatMap (uncurry expandSegments) $ segments xs ps
